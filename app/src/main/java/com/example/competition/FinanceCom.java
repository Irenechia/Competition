...
public class FinanceCom extends AppCompatActivity implements Runnable ,AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_finance_com);
        ...
        handler = new Handler(){
            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                if (msg.what == 1){
                    ...
                    ListView listView = (ListView) findViewById(R.id.lv_comf);
                    ListAdapter adapter = new ArrayAdapter<String>(FinanceCom.this,android.R.layout.simple_list_item_1,liList);
    ...
    public void run() {
        //获取加载时间
        String curDateStrF = (new SimpleDateFormat("yyyy-MM-dd")).format(new Date());
        Log.i(TAG, "curDateStr: "+curDateStrF+"，logDate"+logDateF);

        //判断是从哪里获取数据
        if (curDateStrF.equals(logDateF)){
            //从数据库中获得数据
            Log.i(TAG, "run: 日期相等，从数据库中获得数据");
            InfoManager infoManager = new InfoManager(FinanceCom.this);
            for (InfoItem item:infoManager.listAll(DBHelper.TB_NAME1)){
            ...
            int page = 0;
            try {
                URL url = new URL("https://jinrong.swufe.edu.cn/events.htm");
            ...
            try {
                //首页数据提取
                Log.i(TAG, "run: 首页数据");
                String urlFirst = "https://jinrong.swufe.edu.cn/events.htm";
                ...
                int size1,size2;
                int i = 1;
                String url = "https://jinrong.swufe.edu.cn/events.htm";
                for (;i<page+1;i++){
                    ...
                    for(int m = 46;m<size1-4;m++){
                        ...
                        //Element herText = herfs.get(m);
                        //String infoStr = herText.attr( "title" );
                        //liList.add(infoStr);
                        ...
                        Log.i(TAG, "run: 已添加:"+span.text() +","+herfi.attr( "abs:href" ));
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
            ...
            infoManager.deleteAll(DBHelper.TB_NAME1);
            infoManager.addAll(infoItemsList,DBHelper.TB_NAME1);
            Log.i(TAG, "run: 数据库已更新");
        }
        //记录更新日期
        SharedPreferences sp = getSharedPreferences("fi", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        editor.putString(DATE_SP_KEY,curDateStrF);
        editor.commit();
        Log.i(TAG, "更新数据结束: "+curDateStrF);
    ...
    public void showProgressDialog(String title, String message) {
        if (progressDialog == null) {
            progressDialog = ProgressDialog.show(FinanceCom.this, title, message, true, false);
    ...
    public void search(View btn){
        resultList.clear();
        //获取关键字
        EditText input = findViewById(R.id.key_words_f);
